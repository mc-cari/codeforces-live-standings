...
type CachedResponse = { body: string; status: number };
...
export const createServerSignature = (method: string, parameters: URLSearchParams): string => {
  ...
  if (!apiKey || !apiSecret) throw new Error('Codeforces API credentials are not configured');
...
const fetchCodeforces = async (method: string, parameters: URLSearchParams) => {
  ...
  if (!anonymousMethods.has(method)) {
    const apiSignature = createServerSignature(method, parameters);
...
export const codeforcesApiHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method !== 'GET') {
  ...
  try {
    if (method === 'participant.import') {
      ...
      const { body, status } = await getCodeforcesResponse('contest.standings', standingsParameters);
      if (status !== 200) {
